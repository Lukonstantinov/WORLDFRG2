...
**Status: PLANNED, NOTHING BUILT.** Written 2026-09-23 from a read of the campaign
tick (`sim/campaign/tick/`), not from memory. Every claim in §1 names the code it
...
|---|---|---|---|
| **L0** | **Instrument** `econ_measure_settlement_life` (`#[ignore]`d, on `realm_reference_world` + `dense_world`): implied natural growth by city size; famine episodes and recovery times; plague recovery times; riots/revolts per century; distribution of `lack_basic`; how often `lack_basic > 0.2` while grain stock > 30 days of need (**the F1 count**). | none — diagnostic | runs; baseline row in `SCOREBOARD.md` |
| **L1** | **Entitlement** (= MONEY M7): `food_eaten`/`food_need` recorded, blended into `update_food_and_starvation` (§3.1). Chronicle distinguishes empty-granary from full-warehouse famine. | `ENTITLEMENT_DOSE` 0 → walked | 0: `sim_fingerprint` unchanged. Walk: `unrest_topples_councils` + every famine test still fire; `econ_expenditure_shares_resemble_a_household`; L0's F1 count must RISE (the fix makes hidden hunger visible) while the dynamics run stays bounded. New `a_priced_out_city_reads_as_hungry` (uses S7's `_e` pure function at a test dose). |
| **L2** | **Incomes + welfare ratio** (§3.2), observe only. Scorecard prints `labourer_welfare_ratio`. | none | `sim_fingerprint` unchanged; new `incomes_sum_to_what_the_city_earned` (no income from nowhere); `welfare_ratio_is_finite_and_positive`. |
| **L3** | **Annals + Life tab v1** (§3.12): `CityYear`, `campaign_city_life`, the tab over L0–L2 data; wire `campaign_get_pops`. | UI + observe-only state | `sim_fingerprint` unchanged; `tsc`; `vite build`; looking at it in a running app is OWED where no display exists (the S10 caveat). |
...
| **L4** | **Vital rates + age bands** (§3.3), causes of death, levy deaths from adult males. | `VITAL_RATES_DOSE` 0 → walked, with `CAPACITY_LAND_WEIGHT` pinned | 0: bit-identical. Walk: CBR/CDR/life expectancy printed on the provinced fixture; world population bounded and finite; Zipf and urban share not regressed; new `a_famine_leaves_a_missing_generation`, `war_deaths_fall_on_adult_men`, `big_cities_die_faster_than_they_breed`. |
| **L5** | **Welfare into behaviour** (§3.4). | `WELFARE_BEHAVIOUR_DOSE` | `unrest_topples_councils`; migration gates (`EXODUS_*` tests); new `labourers_riot_before_burghers`. |
| **L6** | **Housing + crowding** (§3.5); real timber/stone demand. | `HOUSING_DOSE` | building-good prices do not blow up (`econ_`); crowding bounded; new `crowding_raises_mortality`, `housing_is_built_from_real_goods`. |
| **L7** | **The settlement year** (§3.6). | `CALENDAR_DOSE` | seasonal mortality peak in the right season by latitude (`summer_fever_in_the_south_winter_deaths_in_the_north`); grain temporal CV PRINTED (expected unmoved until the stock glut is fixed — say so in the SCOREBOARD row). |
