...
**Depends on:** B1 (provinces as the carrier), or can ship standalone at hub level.
...
fix is already half-built: **provinces are the correct join granularity** — hundreds of
units, not millions of cells, partitioned on watersheds and coasts, already carrying
`ProvinceGood { good, quality }`, already generated by `sim_generate_provinces` and
already mapped hub → province.

Do:
- Give `Province` a small **mutable** state vector: forest cover, soil depletion,
  irrigation, cleared land, water table.
- Campaign writes it **once a year** (not per tick).
- Hub production reads it, so land use feeds back into yield.
- Carry the A5 per-year climate anomaly on the same structure.

Cost is `O(provinces)` annually — negligible against a 365-tick year. This is the one
change that turns two simulators into one.

**Gate:** `simulate_decades_reports_dynamics` still bounded and finite; per-tick cost
unchanged within noise (`bench_campaign_tick`).
...
| 4 | **A2** maritime damping | Second measured error (`D→E` 40%); small, targeted |
| 5 | **B1** province coupling | The structural unification; enables A5 |
| 6 | **A5** interannual climate | Cheap once B1 lands; turns climate into a driver of history |
...
There are 34 planning documents in `docs/`. The bottleneck on this project is not
