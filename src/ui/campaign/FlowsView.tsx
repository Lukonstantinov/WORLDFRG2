...
export function FlowsView({ hubId, active, tick, setFlowHighlight }: {
  ...
  const shape = useMemo(() => {
    if (!flows) return null;
    // A PRODUCED good's label is marked (⚒) and gets the gold tint, so the
    // exports/imports key answers "does this city actually grow this, or
    // only pass it through" at a glance — exactly the reading a re-export-
    // heavy entrepôt needs and a volume ranking alone hides.
