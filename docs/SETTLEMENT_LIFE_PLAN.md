...
**Status: L0-L8, L11(shadow), L12(partial) SHIPPED (all at dose 0 where
dosed); L13 MOSTLY SHIPPED.** L0-L2 shipped 2026-09-23, L3 (annals + the Life
tab), L4 (vital rates + age bands), L5 (welfare into behaviour), L6 (housing
& crowding), L7 (the settlement year's seasonal mortality), L8 (fire's toll
on housing and lives), L13's housing/crowding chart, L11 (persistent pops,
shadow phase) and L12 (three of six townspeople roles) in follow-up sessions
— see CLAUDE.md §5.7 for the full account. Housing/crowding (L6) is now
DRAWN in the Life tab (L13). L9/L10 genuinely have no mechanism behind them
yet and are unstarted; L12's other three notable roles (bishop, physician,
watch captain) wait on them. L7 shipped only its FIRST of four named parts
(seasonal mortality) — the lean-months hoarding, harvest-labour dip and
feast-day calendar remain queued, named below. L8 shipped only its FIRST of
three named parts (fire) — flood and water/sanitation remain queued, named
below. L11 is SHADOW ONLY — `PERSISTENT_POPS_DOSE` (which would switch real
readers over) does not exist yet; that is separate, unstarted work, now that
`econ_measure_persistent_pops_divergence` gives it a real baseline to walk
against. L9/L10 remain queued per the build order below. Originally written
from a read of the campaign tick
...
| **L8** | ✅ **SHIPPED, PARTIAL (dose 0)** — Urban hazards (§3.7), first of its three named parts: fire's toll on housing and lives, layered onto the EXISTING warehouse-stock fire event (`kind == "fire"` in `roll_events`) — the stock burn, house-wealth loss, depot damage and estate strike all ship exactly as before, unconditional and unchanged by this dose. `fire_settlement_toll_e` (mod.rs) scales the event's own severity by crowding (a crowded hub loses more to the same blaze), a fixed `timber_share` proxy (1.0 — no per-hub building-material state exists yet; the plan's own "a burned city may enact a stone-rebuilding law" is explicitly queued, not built), and a dry-season risk curve reusing `seasonal_mortality_mult_e`'s warm-climate branch (fire risk peaks at a settlement's own dry/hot season regardless of its general climate). Housing loss is hard-capped (`FIRE_HOUSING_LOSS_CAP`) — a single fire burns A FRACTION, never the whole city; the resulting death rate is a small, fixed share of those displaced (`FIRE_DEATH_RATE_OF_DISPLACED` — the Great Fire of London, 1666, lost ~13,200 houses to a handful of recorded deaths, so fire destroys property far more efficiently than it kills). Deaths tag `CAUSE_FIRE` (reserved since L4, at 0.0 until this slice). REBUILDING DEMAND needs no new code — burning housing below its target simply reopens the deficit L6's `update_housing` already closes (once `HOUSING_DOSE` is raised). Fires/century is now printed (`econ_measure_settlement_life`, journal-filtered on the fire event's own text, the same lower-bound convention riots/revolts/plague strikes already use). Flood (riverine hubs, wet season) and water/sanitation (a new civic structure raising `public_health`) are NOT built this pass, queued. | `URBAN_HAZARD_DOSE = 0.0` (true no-op — `fire_settlement_toll_e` returns exactly `(0.0, 0.0)`; the pre-existing stock/wealth/depot/estate fire effects are unaffected at any dose) | `urban_hazard_dose_zero_is_a_noop`, `fire_toll_is_bounded_and_worse_when_crowded` (`tick::tests`, 304/304); `econ_` 6/6 bit-identical incl. multi-seed inheritance gate. Raising the dose, flood, water/sanitation, and the stone-rebuilding law are each unstarted, separate work. |
...
| **L11** | ✅ **SHIPPED, SHADOW PHASE (§3.10)** — `TickHub.pops_shadow: Vec<Pop>` runs in PARALLEL with `derive_pops` (still drives every real reader): births re-enter a pop's OWN profession, apprenticeship moves labourers into craftsmen, ruin (scaled by `damage`) and famine (scaled by `starving`) move craftsmen/clerks/merchants into labourers, and the shadow's TOTAL reconciles onto real population every year (this pass tracks profession MIX, never net headcount). Migration is NOT modelled — exodus/rural pull carry no profession mix anywhere in the sim, a documented simplification. `pops_shadow` is read by NOTHING in the tick — proven directly (`persistent_pops_shadow_is_never_read_by_the_tick` compares `sim_fingerprint` with the pass called every year vs. never called, identical) — so no dose exists yet, since nothing would read it. `econ_measure_persistent_pops_divergence` (`#[ignore]`d) measures per-profession divergence on both fixtures: craftsmen/soldiers diverge most (~0.04-0.10 mean share), clergy/capitalists least (~0.004-0.008) — the coarse transition rates (`PERSISTENT_POPS_APPRENTICE_RATE`/`_RUIN_RATE`/`_FAMINE_PUSH_RATE`) are named first approximations in their own doc comments, not a calibrated model. | shadow (bit-identical, proven) | `persistent_pops_shadow_moves_people_between_professions`, `persistent_pops_shadow_is_never_read_by_the_tick` (`tick::tests`); `econ_measure_persistent_pops_divergence` (`#[ignore]`d) gives the baseline `PERSISTENT_POPS_DOSE` will need before it can exist. Raising a real dose is unstarted, separate work. |
| **L12** | ✅ **SHIPPED, PARTIAL (§3.11)** — Three of the plan's six named notable roles, the ones with a real institution to anchor to today: Guildmaster (the hub's strongest live `CraftGuild`), Alderman (the council house's own `kin[1]` — a naming read over existing data, no new roll), Agitator (the EXISTING Demagogue `Figure` mechanic, localised — no new roll, no new effect, its unrest bump already fires in `raise_notable_figures`). `TickHub.notables: Vec<Notable>`, capped by `hub.tier` (floored at 1), rebuilt yearly after the guild/Figure passes settle. A fresh appointment is chronicled once (quiet when the roster stands). The bishop (L9), physician (L8's unbuilt water/sanitation half) and watch captain (L10) are explicitly QUEUED, not fabricated — each needs an institution that doesn't exist yet. The ±10% institutional nudge (guildmaster→guild strength, alderman→civic mood) is real but dosed. Served via `campaign_city_notables`, drawn in the Life tab's new "The townspeople" section. | `TOWNSPEOPLE_DOSE = 0.0` (true no-op — the roster/chronicle itself is unconditional, only the ±10% nudge is dosed) | `notable_count_is_bounded_by_tier`, `townspeople_dose_zero_is_a_noop` (`tick::tests`); `econ_` 6/6 bit-identical incl. multi-seed inheritance gate. Bishop/physician/captain, the real nudge dose, and rule 20's milestone-pruning integration are each unstarted, separate work. |
| **L13** | ✅ **MOSTLY SHIPPED** — Life tab v2: pyramid + causes of death (✅), housing/crowding (✅, now drawn — `CityYear.crowding`, tail-aligned per rule 29, snapshotted from real L6 data), the townspeople (✅, L12's three roles). Church/watch (❌, still queued — no mechanism exists; L9/L10). | UI + `CityYear` widened additively (`ages`/`deaths_by_cause`/`crowding`, `#[serde(default)]`) + `campaign_city_notables` (pure derived read) | `tsc` clean; full `tick::tests` (308/308) + `econ_` (6/6 incl. multi-seed inheritance gate) bit-identical — pure bookkeeping onto data L4/L6/L12 already computed, no dose needed for the reads themselves. Church/watch sections still owed a mechanism before they can be shown at all (L9/L10). |
