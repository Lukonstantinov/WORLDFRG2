...
**Status: L0-L2 SHIPPED** (2026-09-23, same day as the plan) — see CLAUDE.md §5.7
for the full account. L3-L13 remain queued per the build order below. Originally
written from a read of the campaign tick (`sim/campaign/tick/`), not from memory;
every claim in §1 names the code it was read from. Coordinates with — and shares
one slice with — `MONEY_AND_COINAGE_PLAN.md` (see §2 D1).
...
| **L3** | **QUEUED** — Annals + Life tab v1 (§3.12): `CityYear`, `campaign_city_life`, the tab over L0–L2 data; wire `campaign_get_pops`. Needed before L2's welfare ratio is visible to anyone (§5 risk 6 — legibility debt); not attempted this session (backend-only session, no frontend/React work done). | UI + observe-only state | `sim_fingerprint` unchanged; `tsc`; `vite build`; looking at it in a running app is OWED where no display exists (the S10 caveat). |
