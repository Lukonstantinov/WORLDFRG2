...
| 2026-09-04 | *this* | — | — | +1 ignored diagnostic (`econ_measure_goods_stock_and_price`) | 0 | **MEASUREMENT ONLY — no production code changed. `docs/CONSUMPTION_AND_GOODS_REVIEW.md`.** Four numbers, all new. (1) **Expenditure shares from the shipped demand tables**: food & drink **12.4%** of a city's consumption spend against a historical 60–80%; luxury-tier goods **69.7%**; gold alone 16.1% and gemstones 12.9%, so a modelled city spends **13.2× more on gems than on bread** and buys 1 unit of gemstones for every 4.5 of wheat. (2) **The warehouse glut, measured**: the reference world holds **312 days of grain and 1,519 days of silk in YEAR ONE**, 1,371/13,277 at year 25, and **7,501 days (20.5 yr) of grain and 62,377 days (171 yr) of silk at year 100** — not drift, a founding condition that compounds. `production ÷ need` settles permanently at 1.0–1.8× for staples and **3.6–6.8× for silk and wine** and never moves toward 1.0. (3) **Prices are pinned at the floor**: `price/base` reads **0.13–0.28** at every date for every good against `PRICE_FLOOR_MULT` = 0.15, so the clamp does all the work and price carries no information. (4) **The manufactory shortage is the same bug**: `maybe_found_guild_workshop` gates on `demand_pressure_at ≥ WORKSHOP_MIN_DEMAND` (1.08), but `demand_pressure_at` is `(price/base).clamp(0.6, 3.0)` — so at a measured 0.17–0.28 it is clamped to 0.6 and the gate is unconditionally false in every city, for every good, from year one. `maybe_found_estate` uses the same figure as a *score* rather than a *gate*, which is exactly why raw estates keep appearing while workshops dry up. Root causes, all traced: consumption is `eat = need.min(stock)` with no counterparty and no budget (**there are no buyers in this economy** — `supply_accum`'s five classes are all sellers); production reads no price anywhere; **31 of 45 goods ship `perishable = 0.0`** so `warehouse_and_spoilage_pass` early-returns and `wh_capacity` (whose only effect is a multiplier *on the spoil rate*) is inert for them; and `need_scale` is **one aggregate scalar** balancing total production against total need across all 45 goods, so at most the average is right and every individual good's level is an accident of two tables that were never compared. Also confirms `tech_factor` is pinned at its 0.85 floor for the whole run (pre-existing, already documented), so the glut is retained surplus, not productivity growth. |
