...
- Each tile has columnar data fields (terrain, elevation, temperature, … plus
  `salinity` u8, `shark_risk` u8, and `goods: Vec<Vec<u8>>` of `GOODS_COUNT`=17
  trade-good belts). New fields are **appended last** in `compress`/`decompress`
...
| 8 | `sim_biological` | Shark-habitat risk + trade-good belts (Biological-Trade step; takes seed) |
...
- `compute_shark_zones` → shark-habitat **cell-mask areas** (overlay: filled cells + outline + 🦈)
- `compute_good_regions` → per-good belt **cell-mask areas** (filled cells + outline + emoji; one toggle per good under Toolbar → Trade Goods)
- `compute_trade_matrix(settlements)` → settlement-cluster regions, per-good
  production/demand/net + region↔region flows (TradeMatrixPanel table + flow-line overlay)
- Trade routes + flows are generated by the **Biological-Trade step** (gated on step 8 in MapCanvas), not auto-recomputed on settlement change.
...
### Trade goods (`sim/biological.rs::compute_trade_goods`, 17 belts, seeded)
`good_score` = climate(Köppen) × temp/precip bands × elevation × fertility × coast
× (fishery/salinity for marine). Then `localize_good` picks ONE suitability-
weighted random seed (deterministic from world seed) and flood-fills the good's
homeland: **land goods** stop at sea + mountains ≥3000 m (`MOUNTAIN_NORM`≈0.339);
**marine goods** have no walls — they spread through coast/sea and stop where the
score envelope (temperature/salinity) drops below threshold. One homeland per good
⇒ one clear producer in the trade matrix. Goods: silk, wine, oliveoil, sugar,
frankincense, stockfish, spices, tea, coffee, furs, timber, amber, salt, dyes,
incense, pearls, whaling (`GOOD_NAMES`; `GOOD_MARINE` flags sea goods).
